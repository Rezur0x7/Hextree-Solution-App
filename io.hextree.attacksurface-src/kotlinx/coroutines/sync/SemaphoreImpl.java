...
public final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                        m1700constructorimpl = SegmentOrClosed.m1700constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m1700constructorimpl = SegmentOrClosed.m1700constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1705isClosedimpl(m1700constructorimpl)) {
                Segment m1703getSegmentimpl = SegmentOrClosed.m1703getSegmentimpl(m1700constructorimpl);
                while (true) {
                    Segment segment = (Segment) this.tail;
                    if (segment.getId() >= m1703getSegmentimpl.getId()) {
                        break loop0;
                    } else if (!m1703getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, m1703getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m1703getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m1703getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m1703getSegmentimpl(m1700constructorimpl);
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (segment.getId() >= m1703getSegmentimpl.getId()) {
                    ...
                    break;
                } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment, m1703getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (m1703getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
