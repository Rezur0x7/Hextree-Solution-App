    ...
    */
    void processTouchRotateEvent(MotionEvent event, MotionLayout.MotionTracker velocityTracker, int currentState, MotionScene motionScene) {
        ...
        if (action == 0) {
            ...
            if (i3 != -1) {
                View findViewById2 = this.mMotionLayout.findViewById(i3);
            ...
            float atan22 = (float) (((Math.atan2(event.getRawY() - height, event.getRawX() - width) - Math.atan2(this.mLastTouchY - height, this.mLastTouchX - width)) * 180.0d) / 3.141592653589793d);
            if (atan22 > 330.0f) {
                atan22 -= 360.0f;
            ...
            if (Math.abs(atan22) > 0.01d || this.mDragStarted) {
                ...
                if (max != progress2) {
                    int i6 = (progress2 > 0.0f ? 1 : (progress2 == 0.0f ? 0 : -1));
                    if (i6 == 0 || progress2 == 1.0f) {
                        this.mMotionLayout.endTrigger(i6 == 0);
                    }
